...
process.source = cms.Source (
    ...
    fileNames = cms.untracked.vstring (
     ...
     #  "file:/afs/cern.ch/work/j/jandrea/public/147DB408-446A-E311-8E63-00259073E32A.root" #Brunel people are doing this..
     ...
     #	"file:/afs/cern.ch/work/m/meshoaib/analysis/CMSSW_5_3_15/src/MyAnalysis/TbZ/A27919CE-486A-E311-8EDD-002590D0B0C8.root"
       "file:/afs/cern.ch/work/m/meshoaib/TupleSingleTop/CMSSW_5_3_11/src/TopQuarkAnalysis/SingleTop/test/A27919CE-486A-E311-8EDD-002590D0B0C8.root"#signalMC
...
if isData: process.source.fileNames= cms.untracked.vstring ("file:/afs/cern.ch/work/m/meshoaib/TupleSingleTop/CMSSW_5_3_11/src/TopQuarkAnalysis/SingleTop/test/003EC246-5E67-E211-B103-00259059642E.root")#DoubleElectron
...
doGsfElectrons = True #False

#Add nJ >= 2 cut: 

addJetsCut= False # True originally it was True

#Run JetMET uncertainties from JME tool

doRunMETUncertainties= True # False

#Enable PF taus (true = default: adds the PF tau objects;false = embed them in jets)
EnablePFTaus= False #True
...
#Tag:
### new GT - 10Sept2013
if isData:  process.GlobalTag.globaltag = cms.string('FT53_V21A_AN6::All') #FT53_V21A_AN6 Brunel mentioned this also

else: process.GlobalTag.globaltag = cms.string('START53_V27::All')    ### Original 

#else: process.GlobalTag.globaltag = cms.string('START53_V26::All') ### Brunel People Used V26 for synchronization
...
process.vetoMuons = process.userDataMuons.clone(
    cut = cms.string(" (isGlobalMuon || isTrackerMuon) " +
                     "& pt >= 10 & abs(eta) <= 2.4 " +
                     "& userFloat(\"DeltaCorrectedIso\") <= 0.2 ")#before <2.5 <0.2
)

process.vetoElectrons = process.userDataElectrons.clone(
    cut = cms.string("ecalDrivenMomentum.pt >= 10 " +
                     "& abs(eta) <= 2.5 " +
                     "& userFloat(\"RhoCorrectedIso\") <0.15"+
		      "& electronID('mvaTrigV0') >0.0")# + before not applied, before pt>20, eta<2.5
...
process.tightMuons = process.userDataMuons.clone(
    cut = cms.string("pt >= 20 & isGlobalMuon && isPFMuon & abs(eta) <= 2.4 && normChi2 < 10 && track.hitPattern.trackerLayersWithMeasurement>5 "+
...
process.tightElectrons = process.userDataElectrons.clone(
    cut =  cms.string("ecalDrivenMomentum.pt >= 20  && abs(eta)<= 2.5" +
                      "& ( abs(superCluster.eta)> 1.5660 || abs(superCluster.eta)<1.4442)" +
                      "& gsfTrack.trackerExpectedHitsInner.numberOfHits <=0" +
                      "& passConversionVeto" +
		      "& electronID(\"mvaTrigV0\") >= 0.5" +	
                      "& userFloat('VertexDxy')<0.04" +
                      "& userFloat('RhoCorrectedIso')<0.12" )
)
...
process.countLeptons.minNumberLoose = 2 # before 0
process.countLeptons.maxNumberLoose = 99 

process.countLeptons.minNumberTight = 2 # before 0
